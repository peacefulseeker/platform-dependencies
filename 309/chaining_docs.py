...
class Document:
    ...
    def add_punctuation(self, punctuation: str, index: int) -> Document:
        ...
        try:
            if any(punctuation for punctuation in EOL_PUNCTUATION if punctuation in self.lines[index]):
                self.lines[index] = re.sub(f'{[EOL_PUNCTUATION]}', punctuation, self.lines[index])
            else:
